...
[X]- Bug: Creatures go inside the player when hit by the player and are clipped to the wall.(Jairo)
[X]-Refactor: We have to fix the folder structure because it really makes no sense and I have been just chugging everything into utils. (Jairo)
[X]-Refactor: Make imports cleaner, there is no proper __init__.py for pakcages and we shouldn't have to use "src." to import things, also the imports are getting outta hand. (Jairo)
